...
For this tutorial, we use the upstream Linux kernel and modify it to simulate a
custom kernel.
...
Now create a new repository for a custom kernel based on the official upstream
Linux kernel, ``6.6.20``.
...
Get the upstream Linux kernel tag corresponding to the version the custom
kernel is based on:
...
upstream one. That the tag is correctly fetched can be checked with
``git tag | grep v6.6.20``.

Define the kernel package configuration
---------------------------------------

Inside the ``custom-linux`` directory, create a ``custom_kernel.toml``
file:
...
The name of the kernel package should always be of the form
``linux-<custom flavour>``, and the version should be of the form
``<kernel_version>-<local_version>``, where ``<kernel_version>`` is the Linux
kernel source on which the custom kernel is currently based.
Also, this tutorial uses the `noble` Ubuntu release.
